...
% RA-CBF for 2D Mobile robot for Position swapping case for Comparison Study
clear; clc; close all
...
%% ═══════════════════════════════════════════════════════════════════════
N_runs      = 25;
N_list      = [8 16];   % ----- set your agent counts here  -----------
dt          = 0.01;
T           = 40;
...
for n_idx = 1:length(N_list)
    ...
    fprintf('\n');
    fprintf('╔══════════════════════════════════════════════════════════════════╗\n');
    fprintf('║   R-SVO CBF  ——  N = %-3d agents                                 ║\n', N);
    fprintf('╠══════════════════════════════════════════════════════════════════╣\n');
    fprintf('║  Runs=%-3d  dt=%.3f  T=%-4.0f  Rsafe=%.2f  gamma=%.1f  k=%.2f      ║\n',...
            N_runs, dt, T, Rsafe, gamma_cbf, k_gain);
    fprintf('╚══════════════════════════════════════════════════════════════════╝\n');

    %% Storage for comparison across conditions
    R_nd = struct(); R_wd = struct();
    ...
    for cond = 1:n_conds
        ...
        for run = 1:N_runs
            ...
            [s, goal] = generateAgentsPolygon(N, R_circle, center);
            x    = s';
            goal = goal';
        ...
        %% Store for No Dist vs With Dist comparison
        if cond == 1
    ...
    %% ─── No Dist vs With Dist comparison for this N ─────────────────
    if run_with_disturbance
        fprintf('╔══════════════════════════════════════════════════════════════════╗\n');
        fprintf('║  R-SVO CBF [N=%-3d]  –  No Disturbance vs With Disturbance      ║\n', N);
        fprintf('╠══════════════════════════════╦═══════════════╦═══════════════════╣\n');
...
function [startPos, goalPos] = generateAgentsPolygon(n, R, center)
    ...
    if n > 100 || n < 0
        error('n must be between 3 and 100');
    end
